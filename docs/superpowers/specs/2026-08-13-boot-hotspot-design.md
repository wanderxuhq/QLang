...
| 项 | 值 |
|---|---|
| A 态(未优化,blob `c4a9f41` = pre-Step-B interpreter.ql;记录时 HEAD~2) | A1=4.833 / A2=4.890s,A 组中位数 ≈ 4.862s |
...
**判定(决策门):** 按规则"B 组每值都显著小于 A 组每值(且 B 组中位数 < A 组中位数 × 0.9)"判"明显变快"。实测 B 组每值确实均小于 A 组每值,但 **B 中位数 4.772 < A 中位数 4.862 × 0.9 = 4.375 不成立**(实际 B ≈ A × 0.982,净提速仅 ≈1.8%);最接近的配对 B3=4.820 与 A1=4.833 仅差 0.013s(≈0.27%),完全落在 ±20% 热漂移内。**结论:未达明显** —— Step B 的 ~10.8 万次分派裁剪确实发生了(计数器见上),但剩余分派裁剪对端到端贡献不足 ~2%,被漂移吞噬。归因推断(未单独实测):解释期大头已被既有 toChars/array-length 优化覆盖。另注:本表 A 态(≈4.86s)与上文「实测基线」6.1s 表为**跨窗口**绝对值比较,差值 ≈1.25s 落在验证策略记载的 ±20%(~1.3s)漂移量级内,不参与决策门判定(决策门只用同窗口 A/B 配对)。后续:**Step A(host `call_function_inner` 裁剪)另行立实现计划**。

## Step A 实测结果(2026-08-13)

| 项 | 值 |
|---|---|
| A 态(未优化 host,plan 起点 BASE `8fa6653` = Step B 最终 HEAD,blob 同) | A1=4.691 / A2=4.756s,A 组中位数 ≈ 4.724s |
| B 态(Task 1+2 已优化 host,HEAD `108c9f4`) | B1=4.725 / B2=4.694 / B3=4.693s,B 组中位数 = 4.694s |
| 同窗口交错 | B1→A1→B2→A2→B3,同一窗口内配对;收尾工作树 == HEAD |
| 字节级回归 | A/B 两态 FULL 输出剥离 12 行 boot banner + `Execution complete.` footer 后逐字节一致;`RESULT: 419/419 asserts passed` 两端一致 |
| 决策门结论 | **未达明显 → 停止 host 机制方向** |

**判定(决策门):** 规则(逐字):"若 B 组每值都显著小于 A 组每值(且 B 组中位数 < A 组中位数 × 0.9),判'明显变快';若组间重叠(漂移吞噬差异),判'未达明显',不跨窗口补测。"

代入数字:A 组 = {4.691, 4.756},中位数 ≈ (4.691+4.756)/2 = 4.7235;阈值 A × 0.9 ≈ 4.251;B 组 = {4.725, 4.694, 4.693},中位数 = 4.694。检查:①"B 组每值都显著小于 A 组每值"**不成立** —— B1=4.725 > A1=4.691(且 B 组三值 4.693–4.725 与 A 组两值 4.691–4.756 区间完全重叠);②"B 中位数 < A 中位数 × 0.9"**不成立** —— 4.694 < 4.251 为假。组间重叠(漂移吞噬差异),**判'未达明显'**。实际 B ≈ A × 0.994(净差 ≈0.6%),落在 ±20% 热漂移内,无证据表明 Task 1(Cow 静态借用 `"unknown"`)+ Task 2(预绑定 `Rc<str>` 参数名)对端到端有可测提升。归因推断(与 Step B 同款纪律,未单独实测):每分派机制开销中 Task 1/2 所裁部分占比很小,叠加解释期分派裁剪对端到端本就 <2%(Step B 已证 ~10.8 万次分派裁剪仅 ≈1.8%),此量级必被漂移吞噬。**结论:Step A 未达"明显变快",停止 host 机制方向,不再沿 host `call_function_inner` 加码**;后续优化另立项,引向 boot 侧数据/分配(受"离开 rust 的摇篮"架构约束,零新 native、数据层不动)。
